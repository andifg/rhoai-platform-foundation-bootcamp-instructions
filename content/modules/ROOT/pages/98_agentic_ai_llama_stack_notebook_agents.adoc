...
Until now we explored the build in agent capabilities of Llama stack via the llama stack python client library. Within this section we are going to use Llama Stack server as an OpenAI API drop in replacement. This functionality is 
very important as it offers the choice to customer to select his prefered agentic framework. 

- Create a new notebook file within your workbench.
...
As the responses API has build in support for tool calls, the model can use it (the mechanism is the same as with the agents we have already seen that the model requests the tool execution and the agent, which is in this case the 
response api of the Llama Stack server)
